function renderResidenceTimeline(data, selectPlace) {
  ...
  const startYear = data.birth_year || data.pohang_move_year || 2010;
  ...
  function draw() {
    ...
    data.rows.forEach((row, i) => {
      ...
      row.periods.forEach(period => {
        ...
        const to = period.end ? time(period.end) : end;
        ...
        const description = period.description + (period.start && !period.approximate ? ` · ${duration}` : '');
